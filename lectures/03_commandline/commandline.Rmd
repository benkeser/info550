...
In your breakout group, figure out how to write one line of code to execute each of the following. 
...
* replaces all instances of the string "doubt" with "uncertainty" (hint: `sed`)

???

`sed` is more flexible than `grep`, but I find the syntax very cryptic and
...
* uses `curl` to download the text of __Romeo and Juliet__ from Project Gutenberg
  * https://bit.ly/romeo_juliet_txt ;
* counts the number of lines for characters Romeo ("Rom."), Juliet ("Jul."), and the Apothecary ("Apoth."); 
* saves a plain text file for each of these characters that says:
