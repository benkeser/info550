...
In the next few steps, we will be running commands to define [IAM roles](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/iam-roles-for-amazon-ec2.html) for your instance. 
...
__Step 2__: Create an [IAM role](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/iam-roles-for-amazon-ec2.html) called `s3access` and link to the trust policy that you created.
...
__Step 3__: Create an access policy that allows permission to use S3 on the instance.
...
We can now launch EC2 instances that have access to AWS S3! 
...
#! /bin/bash
# copy script from s3
aws s3 cp s3://dbenkesers-first-bucket/ec2_script.R ~/
# execute script
chmod +x ~/ec2_script.R
~/ec2_script.R
# copy script output to s3
aws s3 cp ~/output.csv s3://dbenkesers-first-bucket
# turn off instance
sudo poweroff
````
...
Now we can run an instance with our user data. A few important notes:
* We are calling the AMI that we created where `R` and `awscli` are installed.
* Note how we assign the `s3access` role.
* Note how we set the behavior to terminate when we `exit`.

````bash
$ aws ec2 run-instances --image-id ami-0b56ddda752b70ca2 \
--count 1 --instance-type t2.large \
--key-name aws_laptop_key \
--security-group-ids sg-0af9981424cb188b1 \
