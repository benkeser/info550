...
* invite .userb[User B] to be a collaborator on the repository
...
* use `renv::restore()` to synchronize your package library
* use `renv::remove` to remove `wesanderson` from the lockfile
...
.usera[User A]
* review .userb[User B]'s PR and merge it when satisfied
* pull the new code to your local machine
* use `renv::restore()` to synchronize your package library
* confirm that you can build `report.html`
